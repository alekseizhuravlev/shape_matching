...
if __name__ == '__main__':
    ...
    with pyvirtualdisplay.Display(visible=False, size=(1920, 1080)) as disp:
        ...
        single_dataset, pair_dataset = data_loading.get_val_dataset(
            dataset_name, 'test', 200, preload=False, return_evecs=True, centering='bbox'
        )
        
        
        template_path = '/home/s94zalek_hpc/shape_matching/data/SURREAL_full/template/remeshed/template.off'
        
        # mesh_template = trimesh.load(template_path, process=False)
        
        from my_code.datasets.template_dataset import get_template
        
        template = get_template(template_path, 200, [1], 'bbox', return_shot=False)
        ...
        base_path = f'/lustre/mlnvme/data/s94zalek_hpc-shape_matching/figures/template_wise/{dataset_name}'
        ...
        for k in tqdm(range(len(pair_dataset))):
        ...
        # for k in tqdm(range(len(idxs_geo_err))):
            ...
            id_second = data_i['second']['id'].item()
            
            
            # print(id_second, processed_ids)
            
            if id_second in processed_ids:
                continue
            else:
                processed_ids.add(id_second)
            
            
            # p2p_pairzo = torch.tensor(p2p_i['p2p_median_pairzo'])
            
            p2p_median_second = torch.tensor(p2p_i['p2p_median_second'])
            
            
            # print(p2p_median_second.shape, template['evecs'].shape, data_i['second']['evecs'].shape)
            
            Cxy = torch.linalg.lstsq(data_i['second']['evecs'][p2p_median_second][:, :32], template['evecs'][:, :32])[0]
            
            import my_code.utils.zoomout_custom as zoomout_custom
            
            device = torch.device('cuda')
            
            Cxy = zoomout_custom.zoomout(Cxy.to(device), template['evecs'].to(device), data_i['second']['evecs'].to(device), 200 - 32, 1, A2=None).cpu()
            
            
            p2p_rev = fmap_util.fmap2pointmap(Cxy, template['evecs'], data_i['second']['evecs'])
